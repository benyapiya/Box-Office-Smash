...
create temp table us_principals as 
	select title_principals.* 
	from title_principals inner join title_basics_movies_us_all on
		title_principals.tconst=title_basics_movies_us_all.tconst;

alter table us_principals
alter column ordering type integer using ordering::integer;

create table us_principals_by_title as select distinct
	t.tconst,
...
from us_principals as t
left join us_principals as t1 on (t1.ordering=1 and t.tconst=t1.tconst)
left join us_principals as t2 on (t2.ordering=2 and t.tconst=t2.tconst)
left join us_principals as t3 on (t3.ordering=3 and t.tconst=t3.tconst)
left join us_principals as t4 on (t4.ordering=4 and t.tconst=t4.tconst)
left join us_principals as t5 on (t5.ordering=5 and t.tconst=t5.tconst)
left join us_principals as t6 on (t6.ordering=6 and t.tconst=t6.tconst)
left join us_principals as t7 on (t7.ordering=7 and t.tconst=t7.tconst)
left join us_principals as t8 on (t8.ordering=8 and t.tconst=t8.tconst);

create table us_principals_names as
select
	t.tconst,
	t.nconst1,
	...
	t.category8
from us_principals_by_title as t
